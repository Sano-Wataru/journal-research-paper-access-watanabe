%1st paragraph
%1
\revised{An edge computing system was proposed that classifies the postures of people with hand movement working in offices by using a LiDAR sensor network.}
...
%2nd paragraph
%2
\revised{Experiments acquired real LiDAR data from 28 test subjects and three training subjects; preprocessing and deep-learning classification varied data type, LiDAR placement, features, and clustering method.}
\begin{deletedblock}
...
\revised{Validation accuracy reached nearly 100\% for all feature and clustering combinations, with minor differences among clustering methods.}
  %3-1
  \revised{High validation accuracy indicates that PointNet++ learned discriminative patterns from the selected point features under each preprocessing configuration.}
\begin{deletedblock}
...
%4th paragraph
%4
\revised{Test accuracy exceeded the 25\% baseline for four posture classes across feature and clustering combinations; normals without clustering or DBSCAN yielded stable accuracy, and no clustering or DBSCAN outperformed k-means.}
  %4-1
  \revised{Test accuracy above the 25\% baseline confirms generalization beyond the three training subjects for four desk postures.}
%5
\revised{Misclassification between typing and pad operation occurred when both hands rested near the keyboard and trackpad with similar point-cloud layouts.}
  %5-1
  \revised{Clustering methods that discarded sparse hand points increased label bias toward incorrect posture classes.}
